//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//


@interface NSDateFormatter (IBInspectorDateFormatterWidgetIntegration)
- (void)setIbDateFormatterBehavior:(long long)arg1;
- (long long)ibDateFormatterBehavior;
- (void)setRealDateFormat:(id)arg1;
- (id)realDateFormat;
- (void)ibWarnings:(id)arg1 forDocument:(id)arg2 withComputationContext:(id)arg3;
- (id)defaultDesignValueForTableView:(id)arg1;
- (id)ibDefaultImage;
- (id)displayNameForDesignableKeyPath:(id)arg1;
- (id)ibDocumentationKeyPathForKeyPath:(id)arg1;
- (void)ibPopulateKeyPaths:(id)arg1;
- (void)ibPopulateAttributeInspectorClasses:(id)arg1;
@end